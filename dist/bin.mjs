import { i as exportSnapshot, n as renderReport, p as readManifest, r as writeReport, t as REPORT_FILENAME } from "./report-D2k4kC-I.mjs";
...
function parseArgs(argv) {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		if (commands.includes(token) && command === void 0) command = token;
...
function totalRedactions(manifest) {
	return Object.values(manifest.profiles).reduce((sum, profile) => sum + profile.redactions.length, 0) + manifest.home.redactions.length;
}
async function runExport(flags, positional) {
	...
	if (result.warnings.length > 0) {
		console.log("\n警告:");
		for (const warning of result.warnings) console.log(`  - [${warning.kind}]${warning.profile !== void 0 ? ` (${warning.profile})` : ""} ${warning.message}`);
	}
}
async function runInspect(snapshotDir) {
...
async function main() {
	...
	switch (command) {
		...
		case "inspect":
			...
			break;
		case "restore":
			fail("restore lands with the unpack engine (next milestone)");
			break;
